...
class principalmodel extends Model
{
    ...
    function Validar_Cedula($param)
    {
        try {
            ...
            // exit();
            if ($param["IMAGEN"] == null || $param["IMAGECEDULA"] == null) {
        ...
        if ($CONSULTA_API_REG_BIO[0] == 1) {
            ...
            if ($ERROR_FOTO == "No") {
                ...
                if (intval($SIMILITUD) >= 95) {
                    $GUARDAR_DATOS_API_REG_BIO = $this->GUARDAR_DATOS_API_REG_BIO($ID_UNICO_TRANSACCION, $CONSULTA_API_REG_BIO[1], $IMAGEN, $IMAGECEDULA);
    ...
    function GUARDAR_DATOS_API_REG_BIO($ID_UNICO_TRANSACCION, $datos, $IMAGEN, $IMAGECEDULA)
    {
        try {
            $data = $datos["DATOS"][0];
